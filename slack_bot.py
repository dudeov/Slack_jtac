          ...
          if command_str in output_dic[mess_key]:
            ...
            if re.search(search_str, output_dic[mess_key]):
              ...
              if arg_dic['host'] == None or arg_dic['case'] == None:
                ...
                continue
              exec_string = r'/root/jtac/get_data.py ' + arg_dic['host'] + ' ' + arg_dic['case'] + ' ' + '--' + channel_id + r' > /dev/null 2>&1 &'
              sc.api_call("chat.postMessage", channel=channel_id, text = 'Starting uploading logs and RSI from %s to case %s. It will take a while, up to 90 min! I will post the log when I am done.' % (arg_dic['host'], arg_dic['case']), as_user=True) 
              sc.api_call("chat.postMessage", channel=channel_id, text = 'Running comand: %s' % exec_string, as_user=True)
              #os.system(exec_string)
              p = subprocess.Popen(exec_string, shell=True).pid
              print('pid: %s' % p)
